...
def on_a_cube(total_points, string_cord):

    def string_cord_to_nested(string_cord):

        cord_unsplitted = list(map(int, string_cord.split(',')))
        print(cord_unsplitted)
        if len(cord_unsplitted)/3 != total_points:
            print("Wrong input")
            exit()
        
        i=0
        cord=[]
        while i<len(cord_unsplitted):
            cord.append(cord_unsplitted[i:i+3])
            i+=3
    ...
    cord = string_cord_to_nested(string_cord)
    print(cord)
    ...
    for i in range(total_points):
        ...
        if count == 2:
            sum1 = sum1 + abs(sum(cord[i]) - sum(cord[i+1]))
        elif count == 1:
            distance = abs(sum(cord[i]) - sum(cord[i+1]))*1.05
            distance = roundTraditional(distance, 2)
            sum1 = sum1 + distance
        elif count == 0:
            sum1 = sum1 + 0
        else:
            del sum1
            sum1 = 'Contant Developer'
                
    return sum1


print(on_a_cube(3, '1,1,10,2,1,10,0,1,9'))
